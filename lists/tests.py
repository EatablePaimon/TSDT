...
class NewItemTest(TestCase):
    def test_can_save_a_POST_request_to_an_existing_list(self):
        other_list = List.objects.create()
        corrtect_list = List.objects.create()  # 注意：变量名拼写应为correct_list
        ...
        self.assertEqual(Item.objects.count(), 1)  # 断言数据库条目数
        new_item = Item.objects.first()
        self.assertEqual(new_item.text, 'A new item for an existing list')  # 断言文本内容
        self.assertEqual(new_item.list, corrtect_list)  # 断言所属列表
    ...
    def test_redirects_to_list_view(self):
        ...
        self.assertRedirects(response, f'/lists/{corrtect_list.id}/')  # 断言重定向路径